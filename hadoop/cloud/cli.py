...
SPOT_CLOUD_PROVIDER = 'ec2-spot'

DEFAULT_CONFIG_DIR = os.path.join(os.environ['HADOOP_EC2_HOME'], 'conf')
CONFIG_FILENAME = 'ec2-clusters.cfg'
...
def get_cloud_provider(options_dict):
  ...
  if provider ==  DEFAULT_CLOUD_PROVIDER and options_dict.get('price') is not None:
    provider = SPOT_CLOUD_PROVIDER
  return provider
...
def check_launch_options_set(cluster, options):
  print options 
  if cluster.get_provider_code().startswith('ec2'):
