...
class Service(object):
  ...
  def _launch_instances(self, instance_template):
    ...
    instance_ids = self.cluster.launch_instances(it.roles, it.number, it.image_id,
                                            ...
                                            price=it.price,
                                            launch_group=self.cluster.name)
    print "Waiting for %s instances in role %s to start" % \
      (it.number, ",".join(it.roles))
    try:
      self.cluster.wait_for_instances(instance_ids,it.roles)
...
class HadoopService(Service):
  ...
  def launch_slaves(self, instance_template):
    ...
    for role in (NAMENODE, JOBTRACKER):
      singleton_host_env = "%s_HOST=%s" % \
              (self._sanitize_role_name(role), master.private_ip)
  ...
  def launch_cluster(self, instance_templates, config_dir, client_cidr):
    ...
    self._create_client_hbase_site_file(config_dir)
...
echo Proxy pid %s;""" % (process.pid, process.pid)
  ...
  <property>
    ...
    <value>hdfs://%(namenode)s:8020/user/root/GeoIP.dat#GeoIP.dat</value>
  </property>
  <property>
  ...
  """ % {'namenode': namenode.private_ip,
    'jobtracker': jobtracker.private_ip,
    'aws_access_key_id': aws_access_key_id,
    'aws_secret_access_key': aws_secret_access_key})

  def _create_client_hbase_site_file(self, config_dir):
    namenode = self._get_namenode()
    cluster_dir = os.path.join(config_dir, self.cluster.name)
    with open(os.path.join(cluster_dir, 'hbase-site.xml'), 'w') as f:
  ...
  <property>
    ...
    <value>hdfs://%(namenode)s:8020/hbase</value>
  </property>
  <property>
    <name>hbase.zookeeper.quorum</name>
    <value>%(namenode)s</value>
  </property>
  <property>
    <name>hbase.zookeeper.property.server.0</name>
    <value>%(namenode)s:2888:3888</value>
  </property>
  <property>
  ...
  def _wait_for_hadoop(self, number, timeout=1200):
