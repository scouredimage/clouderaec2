...
class Ec2Cluster(Cluster):
  ...
  def print_status(self, roles=None, state_filter="running"):
    ...
    if not roles:
      seen = set()
      for group_name in self._get_all_group_names_for_cluster():
        for instance in self._get_instances(group_name, state_filter):
          if not instance.id in seen:
  ...
  def wait_for_instances(self, instance_ids, roles=None, timeout=600, sleep=5):
  ...
  def terminate(self):
    for group_name in self._get_all_group_names_for_cluster():
      instances = self._get_instances(group_name, "running")
      if instances:
